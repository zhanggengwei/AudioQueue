...
NSString *const VDAudioSessionRouteChangeNotification = @"VDAudioSessionRouteChangeNotification";
NSString *const VDAudioSessionRouteChangeReason = @"VDAudioSessionRouteChangeReason";
NSString *const VDAudioSessionInterruptionNotification = @"VDAudioSessionInterruptionNotification";
NSString *const VDAudioSessionInterruptionStateKey = @"VDAudioSessionInterruptionStateKey";
NSString *const VDAudioSessionInterruptionTypeKey = @"VDAudioSessionInterruptionTypeKey";

static void VDAudioSessionInterruptionListener(void *inClientData, UInt32 inInterruptionState)
{
    ...
    NSDictionary *userInfo = @{VDAudioSessionInterruptionStateKey:@(inInterruptionState),
                               VDAudioSessionInterruptionTypeKey:@(interruptionType)};
    VDAudioSession *audioSession = (__bridge VDAudioSession *)inClientData;
    [[NSNotificationCenter defaultCenter] postNotificationName:VDAudioSessionInterruptionNotification object:audioSession userInfo:userInfo];
}

static void VDAudioSessionRouteChangeListener(void *inClientData, AudioSessionPropertyID inPropertyID, UInt32 inPropertyValueSize, const void *inPropertyValue)
{
    ...
    NSDictionary *userInfo = @{VDAudioSessionRouteChangeReason:@(routeChangeReason)};
    VDAudioSession *audioSession = (__bridge VDAudioSession *)inClientData;
    [[NSNotificationCenter defaultCenter] postNotificationName:VDAudioSessionRouteChangeNotification object:audioSession userInfo:userInfo];
}

@implementation VDAudioSession

+ (id)sharedInstance
{
    static dispatch_once_t once;
    static VDAudioSession *sharedInstance;
    dispatch_once(&once, ^{
...
- (void)dealloc
{
    AudioSessionRemovePropertyListenerWithUserData(kAudioSessionProperty_AudioRouteChange, VDAudioSessionRouteChangeListener, (__bridge void *)self);
}
...
{
    AudioSessionInitialize(NULL, NULL, VDAudioSessionInterruptionListener, (__bridge void *)self);
    AudioSessionAddPropertyListener(kAudioSessionProperty_AudioRouteChange, VDAudioSessionRouteChangeListener, (__bridge void *)self);
}
