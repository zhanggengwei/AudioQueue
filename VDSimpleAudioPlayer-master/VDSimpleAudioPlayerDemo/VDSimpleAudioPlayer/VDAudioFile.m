...
- (BOOL)_openAudioFile
{
    OSStatus status = AudioFileOpenWithCallbacks((__bridge void *)self,
                                        VDAudioFileReadCallBack,
                                        NULL,
                                        VDAudioFileGetSizeCallBack,
                                        NULL,
                                        _fileType,
                                        &_audioFileID);
    if (status != noErr)
...
- (NSArray *)parseData:(BOOL *)isEof
{
    ...
    if (ioNumPackets > 0)
    {
        ...
        for (int i = 0; i < ioNumPackets; ++i)
        {
            ...
            VDParsedAudioData *parsedData = [VDParsedAudioData parsedAudioDataWithBytes:outBuffer + packetDescription.mStartOffset
...
static OSStatus VDAudioFileReadCallBack(void *inClientData,
                                        SInt64 inPosition,
                                        UInt32 requestCount,
                                        void *buffer,
                                        UInt32 *actualCount)
{
    VDAudioFile *audioFile = (__bridge VDAudioFile *)inClientData;
...
static SInt64 VDAudioFileGetSizeCallBack(void *inClientData)
{
    VDAudioFile *audioFile = (__bridge VDAudioFile *)inClientData;
